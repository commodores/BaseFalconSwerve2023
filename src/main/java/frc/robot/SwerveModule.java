...
public class SwerveModule {
    ...
    public void setDesiredState(SwerveModuleState desiredState, boolean isOpenLoop){
        SwerveModuleState state = SwerveModuleState.optimize(desiredState, getState().angle); //Custom optimize command, since default WPILib optimize assumes continuous controller which CTRE is not 
        ...
        double angle = (Math.abs(state.speedMetersPerSecond) <= (Constants.Swerve.maxVelocity * 0.01)) ? lastAngle : state.angle.getDegrees(); //Prevent rotating module if speed is less then 1%. Prevents Jittering.
